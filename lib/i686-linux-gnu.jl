...
"""
struct __JL_Ctag_100
    data::NTuple{4, UInt8}
end

function Base.getproperty(x::Ptr{__JL_Ctag_100}, f::Symbol)
    f === :scheduled && return Ptr{Bool}(x + 0)
    f === :reserved && return Ptr{Csize_t}(x + 0)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_100, f::Symbol)
    r = Ref{__JL_Ctag_100}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_100}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_100}, f::Symbol, v)
    unsafe_store!(getproperty(x, f), v)
end

function Base.propertynames(x::__JL_Ctag_100, private::Bool = false)
